  ...
  if (!auth) return (
    <div style={{ minHeight: '100vh', background: 'var(--gruen)', display: 'flex', alignItems: 'center', justifyContent: 'center', paddingTop: 64 }}>
      <div style={{ width: 320, padding: 40, background: 'rgba(0,0,0,0.25)', borderRadius: 16, border: '1px solid rgba(255,255,255,0.1)' }}>
        <div style={{ fontFamily: "'Russo One', sans-serif", fontSize: 28, color: 'var(--gold)', marginBottom: 8, textAlign: 'center' }}>TRMMLR</div>
    ...
    <div style={{ paddingTop: 80, minHeight: '100vh', background: 'var(--cream)' }}>
      <div style={{ background: 'var(--gruen)', padding: '40px 0 32px' }}>
        <div className="container" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <div style={{ fontFamily: "'Russo One', sans-serif", fontSize: 32, color: 'var(--gold)' }}>Admin</div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 16, marginBottom: 40 }}>
          {[
            ...
            <div key={s.label} className="card" style={{ padding: '20px 24px' }}>
              ...
              <div style={{ fontFamily: "'Russo One', sans-serif", fontSize: 28, color: 'var(--gruen)' }}>{s.value}</div>
            </div>
          ))}
        </div>
        ...
        <div className="card" style={{ overflow: 'hidden', marginBottom: 40 }}>
          <table className="data-table">
            ...
            <tbody>
              {events.map(e => (
                <tr key={e.jahr}>
                  <td style={{ fontFamily: "'Russo One', sans-serif", fontSize: 20, color: 'var(--gold)' }}>{e.jahr}</td>
